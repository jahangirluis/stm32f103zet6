...
#ifdef NET_POS_DEV_REC_POS
static void net_pos_msg_come_cb_func(MessageData *data);
#endif

#ifdef NET_POS_SUPP_REMOTE_CONF
static void net_pos_cfg_come_cb_func(MessageData *data);
#endif

void  net_pos_init_sub_topic(onepos_net_pos_t* pos, const char* dev_id)
{
    prot_sub_t sub_topic[NET_POS_SUB_TOPIC_MAX] = {0,};

    OS_ASSERT(OS_NULL != pos);

    memset((void*)&sub_topic, 0, sizeof(prot_sub_t) * NET_POS_SUB_TOPIC_MAX);

    #ifdef NET_POS_DEV_REC_POS
    snprintf(sub_topic[POS_MSG_SUB_TOPIC].topic_name, ONEPOS_MQTT_TOPIC_STRLEN, "%s%s%s",
             POS_TOPIC_PRE, ONEPOS_DEVICE_ID, NET_POS_MSG_SUB_TOPIC_SUFF);
    sub_topic[POS_MSG_SUB_TOPIC].topic_cb = net_pos_msg_come_cb_func;
    #endif
    #ifdef NET_POS_SUPP_REMOTE_CONF
    snprintf(sub_topic[CONF_SUB_TOPIC].topic_name, ONEPOS_MQTT_TOPIC_STRLEN, "%s%s%s",
             POS_TOPIC_PRE, ONEPOS_DEVICE_ID, NET_POS_CONF_SUB_TOPIC_SUFF);
    sub_topic[CONF_SUB_TOPIC].topic_cb = net_pos_cfg_come_cb_func;
    #endif

    onepos_prot_add_topic(pos->prot, NET_POS_SUB_TOPIC_MAX, sub_topic);
}

void  net_pos_deinit_sub_topic(onepos_net_pos_t* pos)
{
    prot_sub_t sub_topic[NET_POS_SUB_TOPIC_MAX] = {0,};
    ...
    onepos_prot_remove_topic(pos->prot, NET_POS_SUB_TOPIC_MAX, sub_topic);
}

void net_pos_init_pub_topic(onepos_net_pos_t* pos, const char* dev_id)
{
	char	*temp	 = OS_NULL;
    ...
    snprintf(temp, ONEPOS_MQTT_TOPIC_STRLEN, "%s%s%s",
             POS_TOPIC_PRE, ONEPOS_DEVICE_ID, NET_POS_MSG_PUB_TOPIC_SUFF);

	temp = pos->pub_topic[CONF_PUB_TOPIC];
	
    memset((void*)temp, 0, ONEPOS_MQTT_TOPIC_STRLEN);
    snprintf(temp, ONEPOS_MQTT_TOPIC_STRLEN, "%s%s%s",
             POS_TOPIC_PRE, ONEPOS_DEVICE_ID, NET_POS_CONF_PUB_TOPIC_SUFF);

}

os_err_t onepos_wait_net_pos(onepos_net_pos_t *pos, 
                             os_uint32_t timeout_ms,
                             os_bool_t *err_flag)
{
    os_err_t          ret      = OS_EOK;
	os_size_t		  rec_size = 0;
    net_pos_msg_t     comm_msg = {0,};
	
	OS_ASSERT(OS_NULL != pos);
	OS_ASSERT(OS_NULL != err_flag);

    ret = os_mq_recv(pos->pos_data, (void*)&comm_msg, sizeof(net_pos_msg_t) ,os_tick_from_ms(timeout_ms), &rec_size);
    if (OS_EOK != ret && sizeof(net_pos_msg_t) != rec_size)
	{
        ONEPOS_LOG_I("wait network position result error, ret is %d, rec_size is %d!", ret, rec_size);
	}
	else
    {
        memcpy((void*)pos->pos_result, (void*)&comm_msg.position, sizeof(onepos_pos_t));
        *err_flag = comm_msg.err_flag;
    }
    return ret;
}
...
static void onepos_net_pos_exit(onepos_net_pos_t* pos)
{
    ...
    if(OS_FALSE == pos->mut_prot)
    {
        onepos_prot_disconnect(pos->prot);
        onepos_prot_deinit(pos->prot);
        onepos_prot_destroy(pos->prot);

        pos->mut_prot = OS_FALSE;
        pos->prot     = OS_NULL;
    }
    else
    {
    ...
    if (pos->pos_data != OS_NULL)
    {
        os_mq_destroy(pos->pos_data);
        pos->pos_data = OS_NULL;
    }
...
//        if (onepos_net_pos_get_dev_sta())
        // {
            ...
            if (net_pos_func(pos))
            {
#if defined(NET_POS_DEV_REC_POS)
    #if defined(ONEPOS_NET_POS_DEBUG)
                onepos_info_print(&g_sys_net_pos_pos);
    #endif
#endif
            }
            else
...
//        else
//        {
//            ONEPOS_LOG_E("onepos device is not ready!");
//        }
    }
}
 ...
 ***********************************************************************************************************************
 */
static void onepos_net_pos_circ_run(onepos_net_pos_t *pos, onepos_pos_t *pos_src, onepos_prot_t *in_prot)
{
    os_err_t       ret  = OS_EOK;
    ...
    pos->pub_topic = (net_pos_pub_tpc_t*)os_calloc(NET_POS_PUB_TOPIC_MAX, sizeof(net_pos_pub_tpc_t));
	if (pos->pub_topic == OS_NULL)
    {
        ONEPOS_LOG_I("Pls start server after last server stop.");
        goto __exit ;
    ...
    pos->pos_data = os_mq_create("net_pos_mq", sizeof(net_pos_msg_t), 1);
    if (pos->pos_data == OS_NULL)
    {
        ONEPOS_LOG_E("create network position postion data failed");
        goto __exit;
    }
	...
	/* creat onepos protocol */
    if(OS_NULL == in_prot)
    {
        prot = onepos_prot_creat("net_pos");
        onepos_prot_init(prot, ONEPOS_DEVICE_ID, ONEPOS_PASSWORD);
        ret = onepos_prot_connect(prot);
        ONEPOS_LOG_D("network position using new protocol : %s", prot->name);
    }
    else
    {
        prot = in_prot;
        pos->mut_prot   = OS_TRUE;
        ONEPOS_LOG_D("network position using input protocol : %s", prot->name); 
    }
    ...
    if(OS_EOK != ret)
    {
        ONEPOS_LOG_W("onepos protocol connect failed, will exit server.");
        goto __exit;
    }

    net_pos_init_pub_topic(pos, ONEPOS_DEVICE_ID);
    net_pos_init_sub_topic(pos, ONEPOS_DEVICE_ID);
 ...
 ***********************************************************************************************************************
 */
static void onepos_net_pos_sgl_run(onepos_net_pos_t *pos, onepos_pos_t *pos_src, onepos_prot_t *in_prot)
{
    os_err_t       ret  = OS_EOK;
	onepos_prot_t *prot = OS_NULL;
...
static os_bool_t net_pos_parse_rec_data(onepos_pos_t *pos_src, char *data)
{
    ...
    {
        result = (net_pos_parse_pos(pos_src, root)) == OS_EOK ? OS_FALSE : OS_TRUE;
    } 
 ...
 ***********************************************************************************************************************
 */
static os_bool_t net_pos_parse_conf_msg(os_int32_t *config_id, const os_int8_t *data)
{
    cJSON       *root = OS_NULL;
    cJSON       *item = OS_NULL;
...
static void net_pos_cfg_come_cb_func(MessageData *data)
{
    ...
    net_pos_parse_conf_msg(&conf_id, data->message->payload);
...
#if defined(ONEPOS_NET_POS_DEBUG)
    os_kprintf("rec_msg : %s\r\n", (os_int8_t *)data->message->payload);
#endif
    comm_msg.err_flag =
            net_pos_parse_rec_data(&(comm_msg.position), data->message->payload);

    os_mq_send(g_net_pos.pos_data, (void*)&comm_msg, sizeof(net_pos_msg_t), OS_WAIT_FOREVER);    
    memset(data->message->payload, 0, data->message->payloadlen);
}
#endif
...
os_bool_t net_pos_func(onepos_net_pos_t *pos)
{
...
#if defined(ONEPOS_WIFI_POS)
    result = onepos_wifi_pos_pub_msg(json_src);
    if(OS_EOK != result)
    {
    ...
    while(cnt < strlen(pub_msg))
    {
        ...
        if((strlen(pub_msg)) - cnt > 256)
        {
            memcpy(temp, (pub_msg + cnt), 256);
            temp[256] = '\0';
            cnt += 256;
            os_kprintf("%s", temp);
        }
        ...
        {
#if defined(NET_POS_DEV_REC_POS)
		    os_bool_t  err_flag = 0;
            if (OS_EOK == onepos_wait_net_pos(pos, REC_POS_TIMEOUT, &err_flag))
            {
                if(err_flag == OS_TRUE)
                {
                    ONEPOS_LOG_I("parse flag : %d", err_flag);
                    ret = OS_FALSE;
                }
            }
            else
            {
                ONEPOS_LOG_E("wait position from onepos platform is error.");
                ret = OS_FALSE;       
            }
#endif
        }
...
void onepos_start_server(void)
{
    if (ONEPOS_CLOSING == onepos_get_server_sta())
    {
        ...
        {
            onepos_net_pos_sgl_run(&g_net_pos, &g_sys_net_pos_pos, OS_NULL);
        }
        else
        {
            onepos_net_pos_circ_run(&g_net_pos, &g_sys_net_pos_pos, OS_NULL);
        }
    }
}
...
os_bool_t onepos_get_latest_position(onepos_pos_t *src_info)
{
    ...
    if (src_info)
    {
        ret = copy_position(src_info, &g_sys_net_pos_pos);
    }
    else
    {

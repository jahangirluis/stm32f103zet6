...
static int vfs_lfs_mount(struct vfs_mountpoint *mnt_point, unsigned long mountflag, const void *data)
{
    ...
    {
        if (OS_EOK == os_device_open(mnt_point->dev))
        {
            part_info = vfs_lfs_part_info_init(mnt_point->dev);

            if (OS_NULL == part_info)
            {
                ret = -ENOSPC;
                os_device_close(mnt_point->dev);
            }
            else
            {
...
static int vfs_lfs_mkfs(os_device_t *dev)
{
#ifdef LFS_READONLY
    return -EPERM;
#else
    ...
    {
        if (OS_EOK == os_device_open(dev))
        {
            part_info = vfs_lfs_part_info_init(dev);

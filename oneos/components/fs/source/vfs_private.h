...
#define VFS_LOCK_INIT()     os_mutex_init(&vfs_lock, "vfslock", OS_FALSE)
#define VFS_LOCK()          os_mutex_lock(&vfs_lock, OS_WAIT_FOREVER)
#define VFS_UNLOCK()        os_mutex_unlock(&vfs_lock)
...
extern struct vfs_mountpoint *vfs_mount_point_find_and_ref(const char* path);

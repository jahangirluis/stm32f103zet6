...
static int vfs_jffs2_mount(struct vfs_mountpoint* mnt_point, unsigned long mountflag, const void *data)
{
    ...
    device_partition[index].dev = mnt_point->dev;
...
static int vfs_jffs2_unmount(struct vfs_mountpoint *mnt_point)
{
    ...
    for (index = 0; index < DEVICE_PART_MAX; index++)
    {
        if (device_partition[index].dev == mnt_point->dev)
        {
...
static int vfs_jffs2_mkfs(os_device_t *dev_id)
{
    unsigned index;
    ...
    return jffs2_flash_erase_all(dev_id);
}
...
static int vfs_jffs2_open(struct vfs_file *fp)
{
    ...
    result = _find_fs(&mte, mnt_point->dev);
    if (result)
    {
        os_free(jffs2_file);
        return -ENOENT;

...
static int vfs_fat_mount(struct vfs_mountpoint *mnt_point, unsigned long mountflag, const void *data)
{
    ...
    if (os_device_open(mnt_point->dev) != OS_EOK)
    {
        return -EINVAL;
    }
    ...
    /* Save device object. */
    disk_set_devid(index, mnt_point->dev);
    /* Check device sector size. */
    if (os_device_control(mnt_point->dev, OS_DEVICE_CTRL_BLK_GETGEOME, &geometry) == OS_EOK)
    {
 ...
 ***********************************************************************************************************************
 */
static int vfs_fat_mkfs(os_device_t *dev)
{

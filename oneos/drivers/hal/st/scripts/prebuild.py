...
def gen_stm32_link_file_gcc(prj_path, app_addr, app_size):
...
def gen_stm32_link_file_keil(prj_path, app_addr, app_size):
...
def gen_stm32_link_file_iar(prj_path, app_addr, app_size):
...
def gen_stm32_link_file(prj_path):
    ...
    for ss in f1.readlines():
        ...
        if IsDefined('OS_USE_BOOTLOADER') and bootloader_defined == True:
            if "STM32_APP_ADDR" in ss:
                app_addr = ss.split()[2]
        
            if "STM32_APP_SIZE" in ss:
                app_size = ss.split()[2]
    ...
    gen_stm32_link_file_gcc(prj_path, app_addr, app_size)
    gen_stm32_link_file_keil(prj_path, app_addr, app_size)
    gen_stm32_link_file_iar(prj_path, app_addr, app_size)

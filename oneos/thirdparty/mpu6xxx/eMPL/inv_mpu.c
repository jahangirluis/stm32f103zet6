...
extern struct mpu6xxx_device *mpu6xxx_dev;
extern os_err_t mpu6xxx_write_regs(struct mpu6xxx_device *dev, os_uint8_t reg, os_uint8_t len, os_uint8_t *buf);
extern os_err_t mpu6xxx_read_regs(struct mpu6xxx_device *dev, os_uint8_t reg, os_uint8_t len, os_uint8_t *buf);

#define i2c_write(saddr,raddr,len,buf)   mpu6xxx_write_regs(mpu6xxx_dev,raddr,len,buf)
#define i2c_read(saddr,raddr,len,buf)    mpu6xxx_read_regs(mpu6xxx_dev,raddr,len,buf)
#define delay_ms(n)    os_clocksource_ndelay(n*NSEC_PER_MSEC)
#define get_ms(tp)      *tp = os_clocksource_gettime()

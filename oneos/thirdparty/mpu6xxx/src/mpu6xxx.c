 ...
 * @return the writing status, OS_EOK reprensents  writing the value of the register successfully.
 */
os_err_t mpu6xxx_write_regs(struct mpu6xxx_device *dev, os_uint8_t reg, os_uint8_t len, os_uint8_t *buf)
{
    os_int8_t res = 0;
#ifdef OS_USING_I2C
    struct os_i2c_msg msgs;
    ...
    {
#ifdef OS_USING_I2C
        sbuf = os_calloc(1, len+1);
        OS_ASSERT(sbuf != OS_NULL);

        sbuf[0] = reg;
        memcpy(&sbuf[1],buf,len);
        msgs.addr  = dev->i2c_addr;    /* Slave address */
        msgs.flags = OS_I2C_WR;        /* Write flag */
        msgs.buf   = sbuf;             /* Slave register address */
        msgs.len   = len+1;                /* Number of bytes sent */
    ...
    {
#ifdef OS_USING_SPI
        res = os_spi_send_then_send((struct os_spi_device *)dev->bus, &reg, 1, buf, len);
#endif
    }
    return res;
}
 ...
 */
os_err_t mpu6xxx_read_regs(struct mpu6xxx_device *dev, os_uint8_t reg, os_uint8_t len, os_uint8_t *buf)
{
    os_int8_t res = 0;
#ifdef OS_USING_I2C
...
os_err_t mpu6xxx_get_temp(struct mpu6xxx_device *dev, float *temp)
{
    ...
    {
        /* mpu9250:  ((TEMP_OUT �?RoomTemp_Offset)/Temp_Sensitivity)+ 21degC */
        *temp = (double)tmp / MPU9250_TEMP_SEN + MPU9250_TEMP_OFFSET;
    }
    else
    {
        /* icm2060x: ((TEMP_OUT �?RoomTemp_Offset)/Temp_Sensitivity)+ 25degC */
...
struct mpu6xxx_device *mpu6xxx_init(const char *dev_name, os_uint8_t param)
{
    ...
    os_pin_mode(GET_PIN(A,15),PIN_MODE_OUTPUT);
    __HAL_AFIO_REMAP_SWJ_DISABLE();
    os_pin_write(GET_PIN(A,15),0);
    mpu6xxx_write_reg(dev, MPU6XXX_RA_PWR_MGMT_1, 0x80);
    os_clocksource_ndelay(100*NSEC_PER_MSEC);
    mpu6xxx_write_reg(dev, MPU6XXX_RA_PWR_MGMT_1, 0x00);

    mpu6xxx_write_reg(dev,MPU6XXX_RA_GYRO_CONFIG,3<<3);
    mpu6xxx_write_reg(dev,MPU6XXX_RA_ACCEL_CONFIG,0<<3);
    mpu6xxx_write_reg(dev,MPU6XXX_RA_SMPLRT_DIV,19);
    mpu6xxx_write_reg(dev,MPU6XXX_RA_CONFIG,4);

    mpu6xxx_write_reg(dev, MPU6XXX_RA_INT_ENABLE, 0x00);
    mpu6xxx_write_reg(dev, MPU6XXX_RA_USER_CTRL, 0x00);
    mpu6xxx_write_reg(dev, MPU6XXX_RA_FIFO_EN, 0x00);
    mpu6xxx_write_reg(dev, MPU6XXX_RA_INT_PIN_CFG, 0x00);

#endif
    //while(1) os_kprintf("mpu init 1\r\n");
    ...
    //while(1) os_kprintf("mpu init OK:%d\r\n",reg);
    dev->id = reg;

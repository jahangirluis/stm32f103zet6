...
STATIC mp_obj_t machine_pwm_init(mp_obj_t self_in, mp_obj_t freq, mp_obj_t duty) 
{
    ...
    period = 1e9 / pwm_args.frequency;
    pulse = period * ((float)(pwm_args.duty) / 100);
    //os_kprintf("freq:%d, period:%d; duty:%d, pluse:%d\n",pwm_args.frequency,period,pwm_args.duty,pulse);
    //os_kprintf("channel:%d\n",pwm_args.channel);

    ret = self->device->ops->ioctl(self->device, MPY_PWM_CMD_SET_PERIOD, &period);
    if (MP_EOK != ret)
    {
    ...
    ret = self->device->ops->write(self->device->owner.name, pwm_args.channel, &pulse,sizeof(pulse));
    if (MP_EOK != ret)
    {
...
STATIC mp_obj_t machine_pwm_freq(size_t n_args, const mp_obj_t *args) {
	...
	if(n_args == 2)
	{
		...
		int pulse = period * ((float)(self->duty) / 100);

		ret = self->device->ops->write(self->device->owner.name, self->channal, &pulse, sizeof(pulse));
		if (0 != ret)
		{
...
STATIC mp_obj_t machine_pwm_duty(size_t n_args, const mp_obj_t *args) {
	...
	if(n_args == 2)
	{
		int period = 1e9 / self->freq;
		int duty = mp_obj_get_int(args[1]);
		int pulse = period * ((float)(duty) / 100);

		//os_kprintf("freq:%d, period:%d; duty:%d, pluse:%d\n",pwm_obj->freq,period,duty,pulse);
		int ret = self->device->ops->write(self->device->owner.name, self->channal, &pulse, sizeof(pulse));
		if (0 != ret)
		{
			nlr_raise(mp_obj_new_exception_msg_varg(&mp_type_ValueError, "pwm can not set duty\n"));
		}

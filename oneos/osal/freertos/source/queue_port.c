...
BaseType_t xQueueIsQueueEmptyFromISR( const QueueHandle_t xQueue )
{
	...
	if (OS_ADAPT_QUEUE_TYPE_MESSAGEQUEUE == (xQueue->queue_type))
	{
		return (BaseType_t)os_mq_is_empty(QUEUEHANDLE_TO_OSMQ(xQueue));
	}
	else
	{
...
BaseType_t xQueueIsQueueFullFromISR( const QueueHandle_t xQueue )
{
	...
	if (OS_ADAPT_QUEUE_TYPE_MESSAGEQUEUE == (xQueue->queue_type))
	{
		return (BaseType_t)os_mq_is_full(QUEUEHANDLE_TO_OSMQ(xQueue));
	}
	else
	{
...
TaskHandle_t xQueueGetMutexHolder( QueueHandle_t xSemaphore )
{
	...
	if ((OS_ADAPT_QUEUE_TYPE_MUTEX == xSemaphore->queue_type) || (OS_ADAPT_QUEUE_TYPE_RECURSIVE_MUTEX == xSemaphore->queue_type))
	{
        ...
        return (TaskHandle_t)os_container_of(owner, struct QueueDefinition, u);
	}
	else
	{
                                ...
                                TickType_t xTicksToWait )
{
	...
	{
		return xQueueReceive(xQueue, OS_NULL, xTicksToWait);
	}
	else
	{

...

{
    ...
    os_event_recv(TASKHANDLE_TO_OSEVENT(xEventGroup), uxBitsToWaitFor, option, xTicksToWait, &e);
...
{
    os_int32_t e;
    e = os_event_get(TASKHANDLE_TO_OSEVENT(xEventGroup));
    os_event_clear(TASKHANDLE_TO_OSEVENT(xEventGroup), uxBitsToClear);
    return (EventBits_t)e;
}
...
BaseType_t xEventGroupClearBitsFromISR( EventGroupHandle_t xEventGroup,
                                       const EventBits_t uxBitsToClear )
{
    return (BaseType_t)os_event_clear(TASKHANDLE_TO_OSEVENT(xEventGroup), uxBitsToClear);
}
#endif
